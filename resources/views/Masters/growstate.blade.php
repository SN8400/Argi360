...
@section('content')    
    <input type="hidden" id="growStateId" value="{{ $id ?? '' }}">
    <div class="row">
        <div class="col-md-12">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="form-group mb-2">
            <label>To Crop:</label>
            <div id="to_crop_text" class="font-weight-bold">—</div> <!-- JS จะเปลี่ยนข้อความนี้ -->
          </div>
...
<script>
    ...
    function getDataList(){
        let growStateId = $('#growStateId').val();
        if (growStateId) {
            $.ajax({
                url: '/api/growstates/' + growStateId,
                method: 'GET',
    ...
    function loadClonedowns() {
        ...
        $.ajax({
            ...
            async: false, 
            success: function(data) {
                $('#from_crop_id').empty().append(`<option value="">Select Input</option>`);
                $.each(data.data, function(i, item) {
                    if(growStateId == item.id){
                        $('#to_crop_text').text(item.name);
                    }
                    $('#from_crop_id').append(`<option value="${item.id}">${item.name}</option>`);
                });
            }
        });
    }
    ...
    function onCreateState() {
        ...
        let urlMethod = (action === 'update') ? 'PUT' : 'POST';
        let url = (action === 'update') ? `/api/growstates/edit/${id}` : `/api/growstates`;
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        <form action="/units/${row.id}" method="POST" class="align-middle mt-1" onsubmit="return myFunction();">
                            <div class="btn-group btn-group-toggle" data-toggle="buttons">
                                <a href="javascript:void(0)" class="text-decoration-none me-2" onclick="openEditModal(${row.id}, ${row.input_item.id}, \`${row.name}\`, \`${row.code}\`, \`${row.age}\`)">
    ...
    function openEditModal(id, input_item, name, code, age) {
