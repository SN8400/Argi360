...
<form id="chemicalForm">
    @csrf
    <input type="hidden" id="chemicalId" value="{{ $id ?? '' }}">
    ...
    <div class="row align-items-center">
        ...
        <div class="col-md-2 ms-auto">
            <button type="button" class="btn btn-success w-100" onclick="onUpdateChemical()">Update</button>
        </div>
    </div>
</form>
...
$(document).ready(function () {
    ...
    // Load chemical data if editing
    let id = $('#chemicalId').val();
    if (id) {
        $.ajax({
            ...
            success: function (res) {
                if (res.status === 'success') {
                    ...
                    $('textarea[name="details"]').val(item.details);
                    $('input[name="formula_code"]').val(item.formula_code);
                    $('#standard_code_id').val(item.standard_code_id);
                    $('#unit_id').val(item.unit_id);
                    $('input[name="rate_per_land"]').val(item.rate_per_land);
                    $('#bigunit_id').val(item.bigunit_id);
                    $('input[name="package_per_bigunit"]').val(item.package_per_bigunit);
                    $('input[name="ctype"]').val(item.ctype);
                } else {
                    alert('ไม่พบข้อมูล');
                }
            },
